...
for (const key in classes) {
  ...
  describe('ch3-q1: ' + key, function() {
    ...
    it('can push and pop values from middle stack correctly', function() {
      ...
      stack.reverse().forEach(v => chai.expect(this.stack.pop(2)).to.equal(v))
    })
    ...
    it('can push, peek and pop values from all 3 stacks correctly', function() {
      ...
      for (let j = 9; j > 0; --j) {
        for (let i = 1; i <= 3; ++i) {
          ...
          stacks[i - 1].push(val)
          chai.expect(this.stack.peek(i)).to.equal(val)
        }
      }

      for (let i = 1; i <= 3; ++i) {
        stacks[i - 1].reverse().forEach(v => chai.expect(this.stack.pop(i)).to.equal(v))
        chai.expect(this.stack.isEmpty(i)).to.be.true
      }
    })
  })
}
